...
These families are the cases where a mother gave birth to two Cohort Members within the sample selection period. Both of the children are Cohort Members, however they were not part of a multiple birth (like twins and triplets). There are only 10 families where this is the case.
...
It looks like there are about 261 siblings in MCS. I think most are from multiple pregnancies (dualbfam suggests 10 might be from instances where a mother has had more than one child during the recruitment window). For simplicity (and to get an independent dataset and because I don't think 261 will give us sufficient power for any discordant sibling analyses), so far, I have only selected the first child (cohort member a) for the EPoCH dataset. It would be possible to extract data for other cohort members though.

...
Code starts around line 1156:

* child's own smoking/alcohol/caffeine
...
raw_dat <- raw_dat[raw_dat$nocmhh==1,]#select singletons/first borns in cohort (lose 251 + 11)
...
dat$covs_age_child_stage0 <- raw_dat$adaglwa0
dat$covs_age_child_stage0[dat$covs_age_child_stage0<0]<-NA

## can't find age at sweep for other stages (this at stage 0 refers to age at which weight measurement was taken, but I can't find a similar variable for later sweeps. From looking at the MCS documentation, it seems possible that families were interviewed when their children turned a particular age, so we might expect all children to be within a few months of their birthdays - i.e 9 months for sweep 1, 3 for sweep 2, 5 for sweep 3, 7 for sweep 4, 11 for sweep 5. That is what I've assumed below. I checked the likelihood this assumption was correct by looking at how close dat$covs_age_child_stage0 is to 9 months, 61 % were within 1 month of 9 months, 85% within 2 months - so reasonably accurate, but note this variable describes age at last weight measurement NOT age at interview so we would expect some to be <9 months - only 28% were over 9 months, 4.2% are over 10 months, 0.3% over 11 months, so it seems reasonable to assume that the vast majority of interviews took place close to a child's birthday)

dat$covs_age_child_stage2 <- 3
dat$covs_age_child_stage3 <- 5 # sweep 3 (age 5) and 4 (age 7) fall under stage 3, but the sweep 4 datasets did not include weight and height measurements, so we have used sweep 3 only
dat$covs_age_child_stage4 <- 11
...
* Child’s exposure to passive smoke (e.g. amsmkr00)

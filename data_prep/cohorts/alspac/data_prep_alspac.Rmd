...
dads.var.names<-c(
"pb020", # Frequency eating sausages or burgers
"pb021", # Freq eating pies or pastries
"pb022", # freq eating meat
"pb023", # freq eating poultry
"pb024", # freq eating offal
"pb025", # freq eating white fish 
"pb026", # freq eatig oily fish   
"pb027", # freq eating shellfish
"pb028", # freq eating fried food
"pb029", # freq eating cabbage or sim
"pb030", # freq eating carrots
"pb031", # freq eating other veg
"pb032", # freq eating salad
"pb033", # freq eating fresh fruit
"pb036", # freq eating nuts

#coded different:
  
"pb040", # slices of bread or chappatis eaten per day
"pb041a", # eat white bread (binary yes no)
"pb042a", # eat brown/granary
"pb043a", # eat wholemeal binary
"pb044a", # eat chappaits binary
"pb046", #eat bread with fat

"pb047a", # milk in tea
"pb048a", # milk in coffee
"pb049a", # milk on breakfast cereal
"pb050a", # milk as pudding
"pb051a", # milk on its own
"pb052a", # milk as milky drink
...
alspac.vars <-findVars(dads.var.names)

alspac.table<-extractVars(dads.var.names)
...
vars=c(
  "pb020", "pb021", "pb022", "pb023", "pb024", "pb025", "pb026",
  "pb027", "pb028", "pb029", "pb030", "pb031", "pb032", "pb033", 
  "pb036", "pb040", "pb041a", "pb042a", "pb043a", "pb044a", "pb046", "pb047a","pb048a", 
  "pb049a", "pb050a", "pb051a", "pb052a","pb054","pb058")
...
dad.food.data <-alspac.table[,keep_vars]
...
vars<-c("pb020", "pb021", "pb022", "pb023", "pb024", "pb025", "pb026",
        "pb027", "pb028", "pb029", "pb030", "pb031", "pb032", "pb033", 
        "pb036")

dad.food.data[,vars]<-sapply(alspac.table[,vars],recode,
                             "1"=0,
                             "2"=0.5,
                             "3"=2,
                             "4"=5.5,
...
dad.food.data$pb040[alspac.table$pb040<0] <- 1.5
dad.food.data$pb040<-recode(alspac.table$pb040, 
                       "0"=0,
                       "1"=0,
                       "2"=1.5,
                       "3"=3.5,
...
# pb046 slices of bread with fat 
# missing   2 !! why i missing 2 and not NA
# values>7  6
dad.food.data$pb046[alspac.table$pb046<0] <- 2
dad.food.data$pb046[alspac.table$pb046>7] <- 6


# eats types of bread
# 1 Yes 1
# 2 No  0
# !! again missing coded to 0 (no). check
dad.food.data$pb041a[alspac.table$pb041a!=1] <- 0
dad.food.data$pb042a[alspac.table$pb042a!=1] <- 0
dad.food.data$pb043a[alspac.table$pb043a!=1] <- 0


#milk in tea, coffee, pudding, as drink, in milky drink
# !! again missing coded to 0 (no). check
...
# 2 Sometimes           0.5
# 3 Never               0
vars=c("pb047a","pb048a","pb049a","pb050a","pb051a","pb052a") 
...
# variables in dad.food.data now have appropriate frequencies

  
  ###########################################                        
#Set up a data frame for the output: dad.food.gp
...
dad.food.gp=data.frame(aln=alspac.table$aln,
                   qlet=alspac.table$qlet,
                   vegetables=NA,
                   legumes=NA,
                   fruitnuts=NA,
...
###################### Vegetables
# Use variables: 
#   pb029 leafy green    119g
#   pb030 carrots        60g
#   pb032 salad          80g
# pb031 other veg (assuming this is root) 87g
...
dad.food.gp$vegetables <- 119 * dad.food.data$pb029 +
                        60 * dad.food.data$pb030 +
                        80 * dad.food.data$pb032 +
                        87 * dad.food.data$pb031

# these are portions per week - convert to daily:
dad.food.gp$vegetables <- dad.food.gp$vegetables/7


###################### Fruit & Nuts
# pb033 fresh fruit  118g
# pb036 nuts          52g

dad.food.gp$fruitnuts <- 118 * dad.food.data$pb033 +
                          52 * dad.food.data$pb036

# these are portions per week - convert to daily:
dad.food.gp$fruitnuts <- dad.food.gp$fruitnuts/7

###################### Dairy

### milk:
# pb047a  in tea          30g per cup
# pb048a in coffee        30g per cup
# pb049a in cereal        100g per cereal
# pb050a in pudding       150g
# pb051a as a drink       200g
# pb052a in milky drink   260g
# no measure of frequency for these  beyond usually/sometimes -
# previous code has a factor 2/3 which I think is
# interpreting 'usually have a milk drink' as on 2 in 3 days
# 'sometimes' has a value of 0.5, so is thus on 1 in 3 days
...
dad.food.data$milk <- 30 * dad.food.data$pb047a * 2/3 +
                  30 * dad.food.data$pb048a * 2/3 +
                   100 * dad.food.data$pb049a * 2/3 +
  150 * dad.food.data$pb050a * 2/3 +
  200 * dad.food.data$pb051a * 2/3 +
  260 * dad.food.data$pb052a * 2/3
  ...
  ### bread:
  # pb040 portions eaten per day
  
  # Types of bread eaten
  # pb041a white    36g
  # pb042a brown    36g
# pb043a wholemeal    36g
# pb044a chapati    68g
# pb045a Dont usually eat any bread
...
# matrix of bread types eaten: rows are observations, 4 cols for bread types
# matrix contains 1 if type is eaten, 0 otherwise
b.mat <- as.matrix(dad.food.data[,c("pb041a","pb042a","pb043a","pb044a")] )
# count number of different types of bread eaten
n.btype <- apply(b.mat ,1,sum,na.rm=T)
...
# calculate total portion size for a person, given the breads they eat
dad.food.data$bread <- b.mat %*% b.portion 

# and calculate daily amount from portinos eaten per day
dad.food.data$bread <- dad.food.data$bread * dad.food.data$pb040


###################### Meat
# pb020 sausages/burgers 80g
# pb021 pies/pasties    129g
# pb022 red meat         88g
# pb023 poultry          100g
# pb024 offal            98g


dad.food.gp$meat <-  80 * dad.food.data$pb020 +
  129 * dad.food.data$pb021 +
  88 * dad.food.data$pb022 +
  100 * dad.food.data$pb023 +
  98 * dad.food.data$pb024

# these are portions per week - convert to daily:
dad.food.gp$meat <- dad.food.gp$meat/7


###################### Fish

# pb025 white fish   130g
# pb026 oily fish    119g
# pb027 shellfish    89g

food.gp$fish <- 130 * dad.food.data$pb025 +
  119 * dad.food.data$pb026 +
  89 * dad.food.data$pb027

# these are portions per week - convert to daily:
dad.food.gp$fish <- dad.food.gp$fish/7

#!! check below for dad. inclusion etc:
  # Additional code
  
  pheno <- dad.food.gp
pheno$pat.kcal <- alspac.table$c3804 #need to replace this with total energy intake for dads
pheno$pat.kcal[pheno$pat.kcal<0]<-NA
pheno$pat.kcal <- pheno$pat.kcal/4.184 # convert kj to kcal
...
# Vegetables
pheno$veg1000kcal <- (pheno$vegetables / pheno$pat.kcal * 1000)
pheno$vegrmed <- with(pheno, factor(findInterval(pheno$veg1000kcal, c(-Inf, quantile(pheno$veg1000kcal, probs=c(0.33333, .66667),na.rm=T), Inf)), labels=c("Q1","Q2","Q3")))
pheno$vegrmed <- revalue(as.character(pheno$vegrmed), c("Q1"="0", "Q2"="1", "Q3"="2"))
pheno$vegrmed <- as.numeric(pheno$vegrmed)

# Legumes
pheno$leg1000kcal <- (pheno$legumes / pheno$pat.kcal * 1000)
pheno$legrmed <- with(pheno, factor(findInterval(pheno$leg1000kcal, c(-Inf, quantile(pheno$leg1000kcal, probs=c(0.33333, .66667),na.rm=T), Inf)), labels=c("Q1","Q2","Q3")))
pheno$legrmed <- revalue(as.character(pheno$legrmed), c("Q1"="0", "Q2"="1", "Q3"="2"))
pheno$legrmed <- as.numeric(pheno$legrmed)

# Fruit incl nuts	
pheno$fruit1000kcal <- (pheno$fruitnuts / pheno$pat.kcal * 1000)
pheno$fruitrmed <- with(pheno, factor(findInterval(pheno$fruit1000kcal, c(-Inf, quantile(pheno$fruit1000kcal, probs=c(0.33333, .66667),na.rm=T), Inf)), labels=c("Q1","Q2","Q3")))
pheno$fruitrmed <- revalue(as.character(pheno$fruitrmed), c("Q1"="0", "Q2"="1", "Q3"="2"))
pheno$fruitrmed <- as.numeric(pheno$fruitrmed)

# Fish and seafood								
pheno$fish1000kcal <- (pheno$fish / pheno$pat.kcal * 1000)
pheno$fishrmed <- with(pheno, factor(findInterval(pheno$fish1000kcal, c(-Inf, quantile(pheno$fish1000kcal, probs=c(0.33333, .66667),na.rm=T), Inf)), labels=c("Q1","Q2","Q3")))
pheno$fishrmed <- revalue(as.character(pheno$fishrmed), c("Q1"="0", "Q2"="1", "Q3"="2"))
pheno$fishrmed <- as.numeric(pheno$fishrmed)

# Cereals								
pheno$cereal1000kcal <- (pheno$cereals / pheno$pat.kcal * 1000)
pheno$cerealrmed <- with(pheno, factor(findInterval(pheno$cereal1000kcal, c(-Inf, quantile(pheno$cereal1000kcal, probs=c(0.33333, .66667),na.rm=T), Inf)), labels=c("Q1","Q2","Q3")))
pheno$cerealrmed <- revalue(as.character(pheno$cerealrmed), c("Q1"="0", "Q2"="1", "Q3"="2"))
pheno$cerealrmed <- as.numeric(pheno$cerealrmed)

# Meat								
pheno$meat1000kcal <- (pheno$meat / pheno$pat.kcal * 1000)
pheno$meatrmed <- with(pheno, factor(findInterval(pheno$meat1000kcal, c(-Inf, quantile(pheno$meat1000kcal, probs=c(0.33333, .66667),na.rm=T), Inf)), labels=c("Q1","Q2","Q3")))
pheno$meatrmed <- revalue(as.character(pheno$meatrmed), c("Q1"="2", "Q2"="1", "Q3"="0"))
pheno$meatrmed <- as.numeric(pheno$meatrmed)

# Dairy								
pheno$dairy1000kcal <- (pheno$dairy / pheno$pat.kcal * 1000)
pheno$dairyrmed <- with(pheno, factor(findInterval(pheno$dairy1000kcal, c(-Inf, quantile(pheno$dairy1000kcal, probs=c(0.33333, .66667),na.rm=T), Inf)), labels=c("Q1","Q2","Q3")))
pheno$dairyrmed <- revalue(as.character(pheno$dairyrmed), c("Q1"="2", "Q2"="1", "Q3"="0"))
pheno$dairyrmed <- as.numeric(pheno$dairyrmed)

# Olive oil [does not show a normal distribution in ALSPAC, so this code not used]
##pheno$oliveoil1000kcal <- (pheno$oliveoil / pheno$pat.kcal * 1000)
##pheno$oliveoilrmed <- with(pheno, factor(findInterval(pheno$oliveoil1000kcal, c(-Inf, quantile(pheno$oliveoil1000kcal, probs=c(0.33333, .66667),na.rm=T), Inf)), labels=c("Q1","Q2","Q3")))
##pheno$oliveoilrmed <- revalue(as.character(pheno$oliveoilrmed), c("Q1"="0", "Q2"="1", "Q3"="2"))
##pheno$oliveoilrmed <- as.numeric(pheno$oliveoilrmed)
...
names(pheno) <- c("aln","qlet",paste0("diet_father_",names(pheno)[3:ncol(pheno)],"_ever_pregnancy_continuous"))
dat <- left_join(dat,pheno,by=c("aln","qlet"),all.x=T)
...
* PRS for physical activity were only derived for 'sedentary', there are SNPs related to 'overall activity' but the n(=3) is so small it is unlikely to be worthwhile making PRS. The SNP n for sedentary is also very small (n=4) to note .
...
BUT CHECK THIS AGAINST THE SCRIPT ABOVE BEFORE YOU RUN IT BECAUSE THERE MAY HAVE BEEN UPDATES ABOVE THAT AREN'T INCLUDED HERE...
...
#cp<-read_dta("/Volumes/sscm alspac/Data/Current/Other/Cohort Profile/cp_2b.dta")
cp<-read_dta("/Volumes/ALSPAC-Data/Current/Other/Cohort Profile/cp_2b.dta")
...
mz<-read_dta("/Volumes/ALSPAC-Data/Current/Other/Sample Definition/mz_5a.dta") #child sample definition
mz <- mz[mz$mz001==1,] #select core sample pregnancies; n=14,541
mz <- mz[-which(mz$mz010<1),] #remove 69 pregnancies with no known outcome; n=14,472
kz<-read_dta("/Volumes/ALSPAC-Data/Current/Other/Sample Definition/kz_5c.dta") #child sample definition
kz <- kz[kz$kz001==1,] #select core sample children; n=14,676

mult_pregs_core<-read_dta("/Volumes/ALSPAC-Data/Useful_data/Multiple pregs/mult_pregs in alspac_CORE.dta") #definition of multiple pregnancies within cohort
mult_pregs_core<-mult_pregs_core[-which(mult_pregs_core$aln %in% c(36891,45649)),] #remove 36891 from multiple pregnancy file as it doesn't appear in mz or kz but is linked as a sib to 45649 (who does appear in mz)
mult_pregs_core<-left_join(mult_pregs_core,mz,by="aln")
mult_pregs_core<-mult_pregs_core[order(mult_pregs_core$mz024b,mult_pregs_core$mz024a),]#order by year and month of delivery
mult_pregs_core$pregnancy_order_no <-1 #first child
...
meta_data[meta_data=="kb_7a.dta"] <- "kb_7b.dta"
meta_data[meta_data=="kf_8a.dta"] <- "kf_8b.dta"

meta_data <- subset(meta_data,subset=tolower(name) %in% tolower(all_vars))
meta_data <- meta_data[which(meta_data$cat2 %in% c("Clinic","Quest","bestgest","birthweight_centile")|meta_data$cat3 %in% "Samples"),]
meta_data <- meta_data[!duplicated(meta_data$name),]

meta_data1 <- meta_data[meta_data$obj %in% unique(meta_data$obj)[1:10],]
meta_data2 <- meta_data[meta_data$obj %in% unique(meta_data$obj)[11:20],]
meta_data3 <- meta_data[meta_data$obj %in% unique(meta_data$obj)[21:30],]
meta_data4 <- meta_data[meta_data$obj %in% unique(meta_data$obj)[31:length(unique(meta_data$obj))],]

raw_dat1 <- extractVars(meta_data1)
raw_dat2 <- extractVars(meta_data2)
raw_dat3 <- extractVars(meta_data3)
raw_dat4 <- extractVars(meta_data4)

raw_dat <- bind_cols(raw_dat1,
                     raw_dat2[,colnames(raw_dat2) %in% meta_data2$name[-which(meta_data2$name %in% names(raw_dat1))]],
                     raw_dat3[,colnames(raw_dat3) %in% meta_data3$name[-which(meta_data3$name %in% c(names(raw_dat1),names(raw_dat2)))]],
                     raw_dat4[,colnames(raw_dat4) %in% meta_data4$name]
)
...
require(readstata13)
A<-read.dta13("/Volumes/ALSPAC-Data/Current/Quest/Partner/pa_4a.dta")
B<-read.dta13("/Volumes/ALSPAC-Data/Current/Quest/Partner/pb_4b.dta")
C<-read.dta13("/Volumes/ALSPAC-Data/Current/Quest/Partner/pc_3a.dta")
...
# didn't appear in meta_data last time I ran this, so may have to read in file and add manually, like this:
obstet <- read_dta("/Volumes/ALSPAC-Data/Current/Other/Obstetric/OA_r1b.dta")
obstet <- obstet[,c("aln","qlet","HDP","pregnancy_diabetes")]
raw_dat <- left_join(raw_dat,obstet,by=c("aln","qlet"))
...
dad.food.gp$fish <- 130 * dad.food.data$pb025 +
  119 * dad.food.data$pb026 +
  89 * dad.food.data$pb027
...
#!! check below for dad. inclusion etc:
  ...
  pheno <- dad.food.gp
pheno$pat.kcal <- alspac.table$c3804 # this is the wrong variable for total paternal energy intake
pheno$pat.kcal[pheno$pat.kcal<0]<-NA
pheno$pat.kcal <- pheno$pat.kcal/4.184 # convert kj to kcal

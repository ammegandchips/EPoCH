...
raw_dat<-extractVars(dads.var.names)
alspac.table<-raw_dat
...
# 2 Sometimes           0.5
# 3 Never               0
vars=c("pb047a","pb048a","pb049a","pb050a","pb051a","pb052a","pb049a") 
dad.food.data[,vars]<-sapply(alspac.table[,vars], recode,
                         "0"=0,
                         "1"=1,
                         "2"=0.5,
...
dad.food.gp$vegtables <- 119 * dad.food.data$pb029 +
...
# matrix of bread types eaten: rows are observations, 4 cols for bread types
# matrix contains 1 if type is eaten, 0 otherwise
b.mat <- as.matrix(food.data[,c("pb041a","pb042a","pb043a","pb044a")] )
# count number of different types of bread eaten
n.btype <- apply(b.mat ,1,sum,na.rm=T)
...
* PRS for physical activity were only derived for 'sedentary', there are SNPs relared to 'overall activity' but the n(=3) is so small it is unlikely to be worthwhile making PRS. The SNP n for sedentary is also very small (n=4) to note .
...
sensitivity_prs_vars <- names(dat)[grep(names(dat),pattern="sens")]
sensitivity_scaled_prs <- as.data.frame(apply(dat[,sensitivity_prs_vars],2,scale))
names(sensitivity_scaled_prs) <- paste0(sensitivity_prs_vars,"_zscore")
dat <- bind_cols(dat,sensitivity_scaled_prs)
...
meta_data4 <- meta_data[meta_data$obj %in% unique(meta_data$obj)[31:50],]
...
raw_dat <- bind_cols(raw_dat1,
                     ...
                     raw_dat3[,colnames(raw_dat3) %in% meta_data3$name],

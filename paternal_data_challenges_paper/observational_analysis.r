...
bib_all_unique <- bib_all_unique[is.na(bib_all_unique$MotherID)==FALSE,]
...
bib_we_unique <- bib_we_unique[is.na(bib_we_unique$MotherID)==FALSE,]
...
df_wb_bib <- bind_rows(lapply(variables,calc_props,df=bib_we_unique))
...
df_bib <- bind_rows(df_bib,bind_rows(lapply("ethnicity_white",calc_props,df=bib_all_unique)))
...
plot1<-  ggplot(df2)+
  ...
  geom_vline(aes(xintercept = 1), size = .25, linetype = "dashed", colour= "grey40" ) +
       geom_point(aes(x=or,y=1,colour=cohort[1]),size=3,shape=15)+
#  geom_rect(aes(xmin=1,xmax=or,ymin=as.numeric(as.factor(cohort))-.5,ymax=as.numeric(as.factor(cohort))+.5,fill=cohort))+
 #   geom_text(aes(label=round(or,2),x=label_position,y=as.numeric(as.factor(cohort))),size=2.5)+
        geom_errorbarh(aes(xmin=or_lower,xmax=or_upper,y=1,colour=cohort[1]),width=0,size=0.5)+
 #   facet_grid(.~varname, switch = "y",scales="free")+
 #   geom_vline(xintercept = 0,colour="grey40")+
  facet_grid(varname~cohort, switch = "y")+
  labs(title= "Associations between partner participation and\nmother and birth characteristics") +
    theme_grey()+
    xlab("Odds ratio")+
  theme(strip.text.y = element_text(angle = 180),axis.text.x=element_text(size=8),axis.text.y=element_blank(),axis.ticks.y=element_blank(),axis.title.y=element_blank(),panel.grid.minor = element_blank(),panel.grid.major = element_blank(),panel.spacing = unit(.1, "lines"),legend.title=element_blank(),legend.position="none",plot.title = element_text(hjust = 0.5))+
   scale_x_continuous(breaks = c(0.25,0.5,1,2,4) ) +
  coord_trans(x = 'log10')

pdf("mother_characteristics_by_partner_participation.pdf",height=4.1,width=10)
plot1
dev.off()
...
#       							c(summary(lm(scale(anthro_birthweight)~smoking_mother_ever_pregnancy_binary+smoking_father_ever_pregnancy_binary,data=bib_sa_unique))$coef[2,],"Maternal smoking","Complete sample, adjusted for partner smoking","BiB South Asian","Birthweight","Unadjusted for ethnicity"))
# colnames(unadj_bw_bib_sa_res)<-c("estimate","se","t","p","parent","model","cohort","outcome","adjusted")

## BiB WB

# unadj_bw_bib_we_res <- rbind.data.frame(c(summary(lm(scale(anthro_birthweight)~smoking_mother_ever_pregnancy_binary,data=bib_we_unique))$coef[2,],"Maternal smoking","Complete sample","BiB White European","Birthweight","Unadjusted for ethnicity"),
#       							c(summary(lm(scale(anthro_birthweight)~smoking_mother_ever_pregnancy_binary,data=bib_we_unique[bib_we_unique$paternal_participation==1,]))$coef[2,],"Maternal smoking","Sample with participating partners","BiB White European","Birthweight","Unadjusted for ethnicity"),
#       							c(summary(lm(scale(anthro_birthweight)~smoking_mother_ever_pregnancy_binary,data=bib_we_unique[bib_we_unique$paternal_participation==0,]))$coef[2,],"Maternal smoking","Sample with non-participating partners","BiB White European","Birthweight","Unadjusted for ethnicity"),
#       							c(summary(lm(scale(anthro_birthweight)~smoking_mother_ever_pregnancy_binary+smoking_father_ever_pregnancy_binary,data=bib_we_unique))$coef[2,],"Maternal smoking","Complete sample, adjusted for partner smoking","BiB White European","Birthweight","Unadjusted for ethnicity"))
# colnames(unadj_bw_bib_we_res)<-c("estimate","se","t","p","parent","model","cohort","outcome","adjusted")
...
plot2 <- ggplot(data=all_res, aes(x=estimate, y=1,colour=adjusted)) + 
  ...
  labs( y = "", x = "Difference in SD birthweight of offspring of mothers who smoked\nvs those who did not smoke during pregnancy") +
    facet_grid(model~cohort,switch="y") +
  theme_grey()+
  theme(strip.text.y = element_text(angle = 180),axis.text.x=element_text(size=8),axis.text.y=element_blank(),axis.ticks.y=element_blank(),axis.title.y=element_blank(),panel.grid.minor = element_blank(),panel.grid.major = element_blank(),panel.spacing = unit(.1, "lines"),legend.title=element_blank(),legend.position="none",plot.title = element_text(hjust = 0.5))+
  labs(title= "Associations between maternal smoking during pregnancy\nand baby's birthweight") 

pdf("checking_selection_maternal_smoking.pdf",height=2.5,width=8)
plot2
dev.off()

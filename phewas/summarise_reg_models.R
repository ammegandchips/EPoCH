...
summarise_reg_models <- function(exposures,outcomes,model_number,df){
  key_here <- key[which(key$exposure%in%exposures & key$outcome%in%outcomes),c("exposure","outcome","exposure_type",paste0("covariates_",model_number),"person_exposed")]
  list_of_summaries <- apply(key_here,1,function(x){
    exposure <- as.character(x[1])
    outcome <- as.character(x[2])
    adjustment_vars <- na.omit(unlist(strsplit(as.character(x[4]),split=",")))

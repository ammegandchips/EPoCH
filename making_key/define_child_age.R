...
key$child_age_covariates[which((key$outcome%in%c(anthro_outcomes))&(key$outcome_time=="age 8, 9, 10 or 11"))] <-names(dat[grep(names(dat),pattern="stage4_f9|covs_stage4_timepoint")])
...
key$child_age_covariates[which((key$outcome%in%c(sdq_outcomes))&(key$outcome_time=="age 8, 9, 10 or 11"))] <-names(dat[grep(names(dat),pattern="stage4_kq|covs_stage4_timepoint")])
...
key$child_age_covariates[which(key$outcome%in%c(cognitive_outcomes)&key$outcome_time=="age 8, 9, 10 or 11")] <-names(dat[grep(names(dat),pattern="stage4_f8|covs_stage4_timepoint")])

#all mfq (depressive symptoms) outcomes taken from F10 clinic:
key$child_age_covariates[which(key$outcome%in%c(depression_outcomes)&key$outcome_time=="age 8, 9, 10 or 11")] <-names(dat[grep(names(dat),pattern="stage4_f10|covs_stage4_timepoint")])
...
# sdq stage 3 was d
key$child_age_covariates[which((key$outcome%in%c(sdq_outcomes))&(key$outcome_time=="age 3 or 4"))] <-names(dat[grep(names(dat),pattern="age_child_stage2")])
...
key$child_age_covariates[which((key$outcome%in%c(cognitive_outcomes))&(key$outcome_time=="age 5, 6 or 7"))] <-names(dat[grep(names(dat),pattern="age_child_stage3c")])

# school readiness (b)
key$child_age_covariates[which((key$outcome=="neuro_cognition_schoolreadiness_stage2")&(key$outcome_time=="age 3 or 4"))] <-names(dat[grep(names(dat),pattern="age_child_stage2")])

# reading and number skills stage 3 was d
key$child_age_covariates[which((key$outcome=="neuro_cognition_numberskills_stage3")&(key$outcome_time=="age 5, 6 or 7"))] <-names(dat[grep(names(dat),pattern="age_child_stage3d")])
key$child_age_covariates[which((key$outcome=="neuro_cognition_reading_stage3")&(key$outcome_time=="age 5, 6 or 7"))] <-names(dat[grep(names(dat),pattern="age_child_stage3d")])

}

if(cohort=="BIB"|cohort=="bib"|cohort=="BiB"){
  
  key$child_age_covariates[which((key$outcome%in%c(anthro_outcomes))&(key$outcome_time=="age 1 or 2"))] <-names(dat[grep(names(dat),pattern="covs_stage1_timepoint")])

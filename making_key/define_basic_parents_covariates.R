...
key$other_parents_covariates[key$exposure_class =="polygenic risk score" & key$person_exposed=="mother"] <- paste0(colnames(dat)[colnames(dat) %in% c("covs_age_father_pregnancy",paste0("dadpc",1:10))],collapse=",")
key$other_parents_covariates[key$exposure_class =="polygenic risk score" & key$person_exposed=="partner"] <- paste0(colnames(dat)[colnames(dat) %in% c("covs_age_mother_conception",paste0("mumpc",1:10))],collapse=",")

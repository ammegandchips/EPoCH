...
### added: I THINK WE SHOULD PROBABLY ADD BMI HERE TOO (PRE-PREG HEALTH BEHAVIOUR -> PRE-PREG BMI -> CORRELATED WITH HB AND BMI DURING PREG -> OUTCOME) MAYBE ONLY IF THE OUTCOME IS ANTHRO(?)
...
key$extra_parent_covariates[key$person_exposed=="mother"]<-paste(names(dat)[grep(names(dat),pattern="covs_bmi_mother")][1],collapse=",")
...
key$extra_parent_covariates[key$person_exposed=="partner"]<-paste(names(dat)[grep(names(dat),pattern="covs_bmi_father")][1],collapse=",")
